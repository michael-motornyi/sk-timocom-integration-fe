...
import { deleteAllVehicleSpaceOffersAction, generateVehicleSpaceOffersAction } from '@/lib/actions';

interface VehicleSpaceOffersActionsProps {
  hasOffers: boolean;
}

export default function VehicleSpaceOffersActions({ hasOffers }: VehicleSpaceOffersActionsProps) {
  ...
  const handleGenerate = () => {
    startTransition(async () => {
      const result = await generateVehicleSpaceOffersAction(generateAmount);
      if (result.success) {
        setShowGenerateModal(false);
        // Trigger refresh in VehicleSpaceOffersContainer
        window.dispatchEvent(new CustomEvent('refresh-vehicle-space-offers'));
      } else {
  ...
  const handleDeleteAll = () => {
    startTransition(async () => {
      const result = await deleteAllVehicleSpaceOffersAction();
      if (result.success) {
  ...
  return (
    <>
      ...
      <Modal
        ...
        title="Generate Vehicle Space Offers"
      >
      ...
      {showDeleteAll && (
        <Modal
          ...
          title="Delete All Vehicle Space Offers"
        >
          <div className="space-y-4">
            <div className="text-center">
              <p className="text-gray-900 mb-4">
                Are you sure you want to delete all vehicle space offers?
              </p>
